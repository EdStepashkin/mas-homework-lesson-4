    ...
    def stream(self, user_input: str):
        ...
        while iterations < settings.max_iterations:
            ...
            if not getattr(response, "tool_calls", None):
                # Якщо немає інструментів і немає контенту (ознака помилки парсингу великого виводу інструменту)
                if not getattr(response, "content", None) or not str(response.content).strip() or response.content == [{}]:
                    retry_msg = HumanMessage(content="Системна помилка (невалідний формат виклику або порожня відповідь). Спробуй згенерувати звіт знову, ОДНОЗНАЧНО використовуючи інструмент write_report та переконавшись, що форматування JSON правильне.")
                    self.messages.append(retry_msg)
                    yield AIMessage(content="[Система: Виправляємо помилку генерації моделі і намагаємось зберегти ще раз...]")
                    continue
                # Якщо tool_calls немає, але є текст - модель згенерувала фінальну текстову відповідь (небажано, але можливо)
                break
